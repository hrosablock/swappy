...
async def sign_limit_order(web3: AsyncWeb3, private_key, chain_id, verifying_contract, salt, maker_token, taker_token, maker, making_amount, taking_amount, min_return, deadline, partially_able):
    try:
        full_message = {
            "types": {
                "EIP712Domain": [
        ...
        signature = await asyncio.to_thread(web3.eth.account.sign_typed_data, private_key, full_message=full_message)
        return {"orderHash": signature.messageHash.hex(), "signature": signature.signature.hex(), "chainId": str(chain_id), "data": full_message.get("message")}
    except Exception:
...
async def send_limit_order(session, limit_order_request_params: dict):
    ...
    headers = get_headers_params("POST", "aggregator", "/limit-order/save-order", body=json.dumps(limit_order_request_params))

    try:
        async with session.post(url, headers=headers, data=json.dumps(limit_order_request_params)) as response:
            if response.status == 200:
                ...
                logging.error(f"Request failed with status {response.status}: {response_text}")
...
async def create_limit_order(db, user_id, encrypted_key: str, chain_id: int, user_wallet: str, maker_token: str, taker_token: str, making_amount: int, taking_amount: int, min_return: int, deadline_hours: float, partially_able: bool):
    try:
        ...
        async with aiohttp.ClientSession() as session:
            await send_approve_tx(session, web3, user_wallet, spender_address, maker_token, making_amount, private_key, rpc_url, chain_id)
            salt = int(time.time())
            ...
            if signature_params:
                res = await send_limit_order(session, limit_order_request_params=signature_params)
                if res:
                    db.add(EVMLimitOrder.create_limit(chain_id=chain_id, user_id=user_id, salt=salt, maker_token=maker_token, taker_token=taker_token, maker=user_wallet, allowed_sender=ZERO_ADDRESS, making_amount=making_amount, taking_amount=taking_amount, min_return=min_return, deadline=deadline, partially_able=partially_able, order_hash=signature_params.get("orderHash")))
                    await db.commit()
