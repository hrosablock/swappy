...
def get_headers_params(request_method: str, base_path: str, method_name: str, query_params: dict = None, body: dict = None):
    try:
        ...
        sign = base64.b64encode(hmac.new(OKX_SECRET_KEY.encode(), message.encode(), hashlib.sha256).digest()).decode()
...
async def get_allowance(web3: AsyncWeb3, owner_address: str, spender_address: str, token_address: str):
    token_contract = web3.eth.contract(address=web3.to_checksum_address(token_address), abi=allowance_abi)
    try:
        return int(await token_contract.functions.allowance(owner_address, spender_address).call())
    except Exception:
...
async def approve_transaction(session, chain_id: int, from_token: str, amount: str):
    try:
        headers = get_headers_params("GET", "aggregator", "/approve-transaction", {
            "chainId": str(chain_id),
            "tokenContractAddress": from_token,
            "approveAmount": amount,
        })
        url = get_aggregator_request_url("/approve-transaction", {
            "chainId": str(chain_id),
            "tokenContractAddress": from_token,
            "approveAmount": amount,
        })
