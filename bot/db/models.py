...
class User(Base):
    ...
    from_ref: Mapped[Optional[int]] = mapped_column(
    ...
    evm_wallet: Mapped["EVMWallet"] = relationship(
        "EVMWallet", back_populates="user", uselist=False, lazy="joined"
    )
    evm_swaps: Mapped["EVMSwap"] = relationship(
        "EVMSwap", back_populates="user", lazy="selectin"
    )
    evm_limit_orders: Mapped["EVMLimitOrder"] = relationship(
        "EVMLimitOrder", back_populates="user", lazy="selectin"
    )
...
class EVMWallet(Base):
    ...
    @staticmethod
    def create_wallet(
        user_id: int, encrypted_private_key: str, address: str
    ) -> "EVMWallet":
        return EVMWallet(
            user_id=user_id,
...
class EVMLimitOrder(Base):
    ...
    making_amount: Mapped[Numeric] = mapped_column(
        Numeric(precision=100, scale=0), nullable=False
    )
    taking_amount: Mapped[Numeric] = mapped_column(
        Numeric(precision=100, scale=0), nullable=False
    )
    min_return: Mapped[Numeric] = mapped_column(
        Numeric(precision=100, scale=0), nullable=False
    )
    ...
    user: Mapped["User"] = relationship(
        "User", back_populates="evm_limit_orders", lazy="selectin"
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "making_amount >= 0.0", name="check_making_amount_non_negative"
        ),
        CheckConstraint(
            "taking_amount >= 0.0", name="check_taking_amount_non_negative"
        ),
...
class EvmCrosschainSwap(Base):
    ...
    slippage: Mapped[Numeric] = mapped_column(
        Numeric(precision=5, scale=3), nullable=False
    )
    ...
    __table_args__ = (
        CheckConstraint("amount >= 0.0", name="check_swap_amount_non_negative"),
        CheckConstraint(
            "max_price_impact_percent >= 0", name="check_price_impact_non_negative"
        ),
    )
