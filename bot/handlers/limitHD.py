...
async def start_limit_order(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        text="Select a chain for your limit order:", reply_markup=limit_chain_kb()
    )
    ...
    try:
        ...
        if user:
            erc_balances_list, erc_balances_string = await fetch_erc20_balances(
                user.evm_wallet.address, chain_id
            )
            await callback.message.answer(
...
@router.callback_query(
    LimitOrderState.maker_token, F.data.startswith("limit_from_token_")
)
async def set_maker_token(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if is_address(maker_token):
            await callback.message.answer(
                text=f"Now enter the receiving token's contract address:",
    ...
    try:
        ...
        if is_address(maker_token):
            await message.answer(
                "Now enter the receiving token's contract address:",
                reply_markup=cancel_kb(),
            )
            await state.update_data(maker_token=maker_token)
            await state.set_state(LimitOrderState.taker_token)
...
async def set_taker_token(message: Message, state: FSMContext):
    try:
        ...
        if is_address(taker_token):
            await message.answer(
                "Enter the amount you want to sell:", reply_markup=cancel_kb()
            )
...
@router.message(
    LimitOrderState.making_amount, F.text.regexp(re.compile(r"^\d+([.,]\d+)?$"))
)
    ...
    try:
        ...
        if user:
            ...
            if current_balance < int(amount * (10**decimals)):
                ...
                await message.answer(
                    "Enter the amount of receiving token you want to get:",
                    reply_markup=cancel_kb(),
                )
...
@router.message(
    LimitOrderState.taking_amount, F.text.regexp(re.compile(r"^\d+([.,]\d+)?$"))
)
async def set_taking_amount(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            "Now enter the minimum return amount of the swap:", reply_markup=cancel_kb()
        )
...
@router.message(
    LimitOrderState.min_return, F.text.regexp(re.compile(r"^\d+([.,]\d+)?$"))
)
async def set_min_return(message: Message, state: FSMContext):
    try:
        ...
        if current_state.get("taking_amount") <= int(amount * (10**decimals)):
            await message.answer(
                "It can't be greater or equal to taking amount",
                reply_markup=cancel_kb(),
            )
            return

        await message.answer(
            "Now enter for how many hours the order will be active",
            reply_markup=cancel_kb(),
        )
...
@router.message(
    LimitOrderState.deadline_hours, F.text.regexp(re.compile(r"^\d+([.,]\d+)?$"))
)
async def set_deadline_hours(message: Message, state: FSMContext):
    try:
        ...
        if hours > 1000 or hours < 0.016:
            await message.answer(
                "The number must be less than 1000 and more than 0.016",
                reply_markup=cancel_kb(),
            )
            return

        await message.answer(
            "Can order be partially filled?", reply_markup=limit_yes_no_kb()
        )
...
async def set_partially_able(callback: CallbackQuery, state: FSMContext):
    try:
        partially_able = (
            True if callback.data.removeprefix("partially_") == "yes" else False
        )
...
async def confirm_limit_order(
    ...
    try:
        ...
        if user:
            ...
            if order_hash:
                await callback.message.answer(
                    f"Limit order created! Order hash: {html.code(order_hash)}",
