...
@router.message(CommandStart(deep_link=True, magic=F.args.regexp(re.compile(r"id_(\d+)"))))
async def ref_start_handler(message: Message, command: CommandObject, db: AsyncSession, state: FSMContext) -> None:
    try:
        await state.clear()
        ...
        if user is None:
            ...
            first_evm_wallet = EVMWallet.create_wallet(user_id=user_id, encrypted_private_key=enc_private_key, address=address)
            db.add(first_evm_wallet)
            await db.commit()
            await message.answer(text=f"Your private key: {html.spoiler(decrypt_key(enc_private_key))}")
            user = await get_user_by_id(db, user_id)
...
async def start_handler(message: Message, db: AsyncSession, state: FSMContext) -> None:
    try:
        ...
        if not user:
            ...
            db.add(user)
            enc_private_key, address = evm_generator()
            first_evm_wallet = EVMWallet.create_wallet(user_id=message.from_user.id, encrypted_private_key=enc_private_key, address=address)
            db.add(first_evm_wallet)
            await db.commit()
            await message.answer(text=f"Your private key: {html.spoiler(decrypt_key(enc_private_key))}")
            user = await get_user_by_id(db, user_id)
...
async def callback_wallet(callback: CallbackQuery, db: AsyncSession):
    try:
        ...
        if user:
            await callback.message.answer(f"Your wallet address: {html.code(user.evm_wallet.address)}\n\nPrivate key: {html.spoiler(decrypt_key(user.evm_wallet.encrypted_private_key))}", reply_markup=menu_kb())
        else:
