...
async def start_swap(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        text="Let's swap!\nFirst, select a chain from the list:",
        reply_markup=swap_chain_kb(),
    )
...
@router.callback_query(
    SwapState.chain_id,
    F.data.startswith("swap_chain_") & F.data.removeprefix("swap_chain_").isdigit(),
)
    ...
    try:
        ...
        if user:
            native_coin_balance = await get_balance(
                chain_id, user.evm_wallet.address, evm_native_coin
            )
...
@router.message(
    SwapState.slippage_percent, F.text.regexp(re.compile(r"^\d+([.,]\d+)?$"))
)
...
@router.message(
    SwapState.price_impact_percent, F.text.regexp(re.compile(r"^\d+([.,]\d+)?$"))
)
async def set_price_impact_percent(message: Message, state: FSMContext) -> None:
    try:
        ...
        if 0.1 <= price_impact <= 51:
            ...
            await message.answer(
                "Invalid price impact percentage. Enter a value between 0.1 and 51",
                reply_markup=cancel_kb(),
            )
    ...
    try:
        ...
        if user:
            ...
            tx_hash = await swap(
                ...
                chain_id=swap_data.get("chain_id"),
                amount=swap_data.get("amount"),
                from_token=swap_data.get("from_token"),
                to_token=swap_data.get("to_token"),
                wallet_address=user.evm_wallet.address,
                price_impact_percent=swap_data.get("price_impact_percent"),
                slippage_percent=swap_data.get("slippage_percent"),
            )
            if tx_hash:
                await callback.message.answer(
                    f"Swap tx initiated\n{html.code(tx_hash)}\n\n{chain_id_to_tx_scan_url.get(swap_data.get('chain_id'))}{tx_hash}",
