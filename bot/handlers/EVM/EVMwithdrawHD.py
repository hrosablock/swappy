...
async def start_withdraw(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        ...
        reply_markup=withdraw_chain_kb(),
    )
    await callback.answer()


@router.callback_query(
    WithdrawState.chain_id,
    F.data.startswith("withdraw_chain_")
    & F.data.removeprefix("withdraw_chain_").isdigit(),
)
...
@router.callback_query(
    WithdrawState.token_address, F.data.startswith("withdraw_token_")
)
async def set_token_address(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if is_address(token_address):
            await callback.message.answer(
                text="Now enter the amount you want to withdraw:",
...
async def confirm_withdraw(
    ...
    try:
        ...
        if tx_hash:
            await callback.message.answer(
                f"Withdrawal initiated. Transaction hash: {html.code(tx_hash)}\n\n{chain_id_to_tx_scan_url.get(chain_id)}{tx_hash}",
        ...
        await callback.message.answer(
            "Something went wrong during withdrawal.", reply_markup=menu_kb()
        )
        logging.exception(f"Error in {sys._getframe().f_code.co_name}: {e}")
    finally:
        await state.clear()
