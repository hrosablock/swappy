...
async def start_crosschain_swap(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        text="Let's perform a crosschain swap!\nFirst, select the source chain:",
        reply_markup=crosschain_from_chain_kb(),
    )
    await callback.answer()


@router.callback_query(
    CrosschainState.from_chain,
    F.data.startswith("crosschain_from_chain_")
    & F.data.removeprefix("crosschain_from_chain_").isdigit(),
)
async def set_from_chain(callback: CallbackQuery, state: FSMContext) -> None:
    try:
        ...
        await callback.message.answer(
            text="Now, select the destination chain:",
            reply_markup=crosschain_to_chain_kb(),
        )
...
@router.callback_query(
    CrosschainState.to_chain,
    F.data.startswith("crosschain_to_chain_")
    & F.data.removeprefix("crosschain_to_chain_").isdigit(),
)
async def set_to_chain(
    ...
    try:
        ...
        if user:
            native_coin_balance = await get_balance(
                from_chain, user.evm_wallet.address, evm_native_coin
            )
            ...
            erc_balances_list, erc_balances_string = await fetch_erc20_balances(
                user.evm_wallet.address, from_chain
            )
            await callback.message.answer(
                text=f"Please provide the token address or select from available tokens:\n\n{native_token_name}: {formatted_balance}\n{html.code(evm_native_coin)}{erc_balances_string}",
...
@router.callback_query(
    CrosschainState.from_token, F.data.startswith("crosschain_token_")
)
async def set_from_token(callback: CallbackQuery, state: FSMContext) -> None:
    try:
        ...
        if is_address(from_token):
            await callback.message.answer(
                text=f"Now enter the receiving token's contract address in destination chain:",
    ...
    try:
        ...
        if user:
            ...
            if current_balance < int(amount * (10**decimals)):
                ...
                await message.answer(
                    "Now enter the slippage % (0.1 to 10)", reply_markup=cancel_kb()
                )
...
@router.message(
    CrosschainState.slippage_percent, F.text.regexp(re.compile(r"^\d+([.,]\d+)?$"))
)
async def set_slippage_percent(message: Message, state: FSMContext) -> None:
    try:
        ...
        if 0.1 <= slippage <= 10:
            await message.answer(
                "Now enter the price impact % (0.1 to 51)", reply_markup=cancel_kb()
            )
            await state.update_data(slippage_percent=slippage)
            await state.set_state(CrosschainState.price_impact_percent)
        else:
            await message.answer(
                "Invalid slippage percentage. Enter a value between 0.1 and 10",
                reply_markup=cancel_kb(),
            )
...
@router.message(
    CrosschainState.price_impact_percent, F.text.regexp(re.compile(r"^\d+([.,]\d+)?$"))
)
async def set_price_impact_percent(message: Message, state: FSMContext) -> None:
    try:
        ...
        if 0.1 <= price_impact <= 51:
            ...
            await message.answer(
                "Invalid price impact percentage. Please enter a value between 0.1 and 51",
                reply_markup=cancel_kb(),
            )
...
async def confirm_crosschain_swap(
    callback: CallbackQuery, state: FSMContext, db: AsyncSession
) -> None:
    try:
        user = await get_user_by_id(db, callback.from_user.id) or await registration(db, callback.from_user.id, callback.message)
        if user:
            ...
            if tx_hash["ok"]:
                ...
                await db.commit()
                await callback.message.answer(
                    f"Crosschain swap initiated! Transaction hash: {html.code(tx_hash.get('tx_hash'))}\n\n{chain_id_to_tx_scan_url.get(swap_data.get('from_chain'))}{tx_hash.get('tx_hash')}",
                    reply_markup=menu_kb(),
                )
            else:
                await callback.message.answer(
                    f"Error: {tx_hash.get('message')}", reply_markup=menu_kb()
                )
