...
from bot.utils.balances import get_ton_balance, fetch_jetton_balances
from bot.utils.token_details import ton_address_validation, get_jetton_decimals
...
async def start_withdraw(callback: CallbackQuery, state: FSMContext, db: AsyncSession) -> None:
...
async def set_token(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        if ton_address_validation(token) or token.upper() == ton_native_coin:
            ...
            await callback.message.answer("Now enter destination address:", reply_markup=cancel_kb())
...
async def set_destination(message: Message, state: FSMContext):
    ...
    if ton_address_validation(destination):
        ...
        await message.answer("Now enter the amount to withdraw:", reply_markup=cancel_kb())
...
async def confirm_withdraw(callback: CallbackQuery, state: FSMContext, db: AsyncSession) -> None:
    try:
        user = await get_user_by_id(db, callback.from_user.id)
        if not user:
        ...
        if tx_hash:
            await callback.message.answer(f"Withdrawal initiated\nhttps://tonviewer.com/transaction/{tx_hash}", reply_markup=menu_kb())
        else:
            await callback.message.answer(f"Withdrawal failed. Please check your balance or try again later.", reply_markup=menu_kb())
    except Exception as e:
        await callback.message.answer(f"Withdrawal failed. Please check your balance or try again later.", reply_markup=menu_kb())
        logging.exception(f"Error in {sys._getframe().f_code.co_name}: {e}")
    finally:
