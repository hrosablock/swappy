...
async def start_nft_purchase(callback: CallbackQuery, state: FSMContext, db: AsyncSession) -> None:
...
async def set_collection(message: Message, state: FSMContext):
    ...
    await message.answer("Now enter the maximum price you are willing to pay for the NFT:", reply_markup=cancel_kb())
...
async def set_price(message: Message, state: FSMContext, db: AsyncSession):
    try:
        ...
        ton_balance = await get_ton_balance(user.ton_wallet.address, user.ton_wallet.encrypted_mnemonic)
        if not ton_balance.get("ok") or float(ton_balance.get("balance") / 10**9) < price+0.6:
            await message.answer("Insufficient balance.(add 0.6 TON for chain fees)", reply_markup=cancel_kb())
            return
...
async def confirm_purchase(callback: CallbackQuery, state: FSMContext, db: AsyncSession) -> None:
    try:
        user = await get_user_by_id(db, callback.from_user.id)
        if not user:
